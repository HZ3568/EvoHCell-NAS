...
from bananas.params import *


def run_experiments(args, save_dir):
    os.environ['search_space'] = args.search_space  # 通过环境变量传递搜索空间类型

    from bananas.nas_algorithms import run_nas_algorithm
    ...
    ss = mp.pop('search_space')  # 删除并返回值
    dataset = mp.pop('dataset')
    search_space = Data(ss, dataset=dataset)  # search_space是一个Data对象
    ...
    for i in range(trials):
        ...
        for j in range(num_algos):
            ...
            run_datum = run_nas_algorithm(algorithm_params[j], search_space, mp)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--trials', type=int, default=1, help='Number of trials')  # 原始为500
    parser.add_argument('--search_space', type=str, default='darts', help='nasbench or darts')

    # test; test_simple; random; bananas; main_experiments; ablation
