...
class Architect(object):
    ...
    def step_v2(self, input_train, target_train, input_valid, target_valid, lambda_train_regularizer):
        ...
        self.optimizer.zero_grad()  # 0.00004s - 0.00006s
        logits = self.model(input_train)  # 1.1s - 1.2s
        loss_train = self.criterion(logits, target_train)  # 0.00005s - 0.00007s
        arch_parameters = self.model.arch_parameters()  # 0.000002s - 0.000004s
        grads_alpha_with_train_dataset = torch.autograd.grad(loss_train, arch_parameters)  # 0.8s - 0.9s

        # grads_alpha_with_val_dataset
        self.optimizer.zero_grad()
        logits = self.model(input_valid)
        loss_val = self.criterion(logits, target_valid)  # logits: 16(batch_size)*10, target_valid: 1*10
        arch_parameters = self.model.arch_parameters()
